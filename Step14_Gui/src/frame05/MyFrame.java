...
public class MyFrame extends JFrame implements ActionListener{
	...
	public MyFrame(String title) {
		...
		// ActionListener 인터페이스 type 의 참조 값을 얻어내서
		ActionListener listener1 = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// JOptionPane.showMessageDialog(프레임의 참조 값, 띄울 메세지);
				// 클래스명.this하면 바깥에 있는 클래스로 생성한 객체의 참조 값을 가리킬 수 있다.
		...
		*/
		...
		// ActionListener 인터페이스 type의 참조 값을 함수 형태로 전달 가능
		deleteBtn.addActionListener((e)->{
			System.out.println("삭제 버튼 눌렀네?");
			// 함수 형태의 메소드에서 this는 바깥 클래스로 생서한 객체의 참조 값을 가리킬 수 있다.
			JOptionPane.showMessageDialog( this, "삭제합니다.");
		});
		
		JButton updateBtn=new JButton("수정");
		add(updateBtn);
		// MyFrame을 ActionListener 인터페이스 type으로 만들 수 는 없을까?
