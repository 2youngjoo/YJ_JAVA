...
public class QuizFrame extends JFrame implements ActionListener{
   ...
   public void actionPerformed(ActionEvent e) {
      ...
      if(cmd.equals("add")) { //만일 추가 버튼을 눌렀다면
         ...
         dto.setName(name);
         dto.setAddr(addr);
         // 3. FriendDao 객체의 insert() 메소드를 이용해서 DB 에 실제 저장하고 
         boolean isSuccess=new FriendDao().insert(dto);
         // 4. 저장 성공이면 "저장 했습니다!" 를 알림에 띄우고
         if(isSuccess) {
            JOptionPane.showMessageDialog(this, "저장했습니다");
            // 5. JTable 에 목록이 다시 출력 되도록 한다.
            displayMember();
         }
         ...
         if(result==JOptionPane.YES_OPTION) {       	 
        	 // 선택된 row 에 해당하는 번호(PK) 를 얻어낸다 
        	 int num=(int)model.getValueAt(selectedRow, 0);
        	 // FriendDao 객체를 이용해서 정보를 삭제한다.
   ...
   public void displayMember() {
      ...
      //회원목록을 얻어오기
       ...
       for(FriendDto tmp:list) {
          // FriendDto 객체 하나당 Object[] 을 하나씩 만들어내서 
